...
int main(int argc, char** argv) {
  ...
  void CreateArchRefHeader() {
    ...
    extern int HaveFormattedRegs, HaveMultiCycleIns, HaveMemHier,
                  HaveTLMIntrPorts, HaveTLM2IntrPorts;
    ac_sto_list *pstorage;
    ac_stg_list *pstage;
    char Globals[5000];
    char *Globals_p = Globals;
...
/*   } */
  ...
  /* ATTENTION: THIS CODE IS NECESSARY?
  if( HaveMemHier ) {
...
void CreateArchImpl() {
  ...
  extern int HaveMultiCycleIns, HaveMemHier, HaveTLMPorts, HaveTLMIntrPorts,
              HaveTLM2Ports, HaveTLM2NBPorts, HaveTLM2IntrPorts, reg_width;
...
//   fprintf(output, "%sfor (;;) {\n\n", INDENT[1]);
  ...
  /*ATTENTION: archc 2.2 withou cache
   *
   * if( HaveMemHier ){
    for( pstorage = storage_list; pstorage!= NULL; pstorage = pstorage->next ) {
...
/***************************************/
void EmitCacheDeclarationRemoved
( FILE *output, ac_sto_list* pstorage, int base_indent){
